...
export default function Edit({ attributes, setAttributes }) {
	...
	const blockProps = useBlockProps({
		...
		style: {
			"--return-bg": attributes.backgroundColor || "#f8f7f4",
			"--return-color": attributes.textColor || "#000000",
			"--return-muted": attributes.mutedColor || "#8f8a83",
			"--return-card-bg": attributes.cardBackgroundColor || "#ffffff",
			"--return-accent": attributes.accentColor || "#f16a6c",
			"--return-max-width": `${attributes.maxWidth || 1180}px`,
			"--return-padding-top": `${attributes.paddingTop ?? 72}px`,
			"--return-padding-bottom": `${attributes.paddingBottom ?? 86}px`,
			"--return-margin-top": `${attributes.marginTop ?? 0}px`,
			"--return-margin-bottom": `${attributes.marginBottom ?? 0}px`,
		},
	});
	...
	return (
		<>
			<InspectorControls>
				...
				<PanelBody title="Colors" initialOpen={false}>
					{[
						...
						<div className="return-policy-editor__color" key={key}>
							<p>{label}</p>
							<ColorPalette colors={COLORS} value={attributes[key]} onChange={(value) => setAttributes({ [key]: value })} />
						</div>
					))}
				</PanelBody>
				<PanelBody title="Return Sections" initialOpen={false}>
					{sections.map((section, sectionIndex) => (
						<div className="return-policy-editor__group" key={sectionIndex}>
			...
			<section {...blockProps}>
				<div className="return-policy__inner">
					<header className="return-policy__header">
						<RichText tagName="p" className="return-policy__eyebrow" value={attributes.eyebrow} onChange={(eyebrow) => setAttributes({ eyebrow })} placeholder="Eyebrow" />
						<RichText tagName="h1" className="return-policy__heading" value={attributes.heading} onChange={(heading) => setAttributes({ heading })} placeholder="Page heading" />
						<RichText tagName="p" className="return-policy__description" value={attributes.description} onChange={(description) => setAttributes({ description })} placeholder="Description" />
					</header>
					<div className="return-policy__list">
						{sections.map((section, index) => (
							<article className="return-policy__section" key={index}>
								<div className="return-policy__number">{String(index + 1).padStart(2, "0")}</div>
								<div className="return-policy__content">
									<RichText tagName="h2" className="return-policy__title" value={section.title} onChange={(title) => updateSection(index, { title })} placeholder="Section title" />
